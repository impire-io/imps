...
//
// Per the constitution's "Imps see one subject path" principle (v2.2.0),
// the harness performs no subject transformation: a declared subject is
// the substrate subject verbatim.
...
// bindSubjectChannel registers a core-NATS subscription on the declared
// subject verbatim. The handler runs dispatch on a fresh per-message
// context.
func (i *Imp) bindSubjectChannel(spec ChannelSpec, src SubjectSource) error {
	state := &channelState{spec: spec, subject: src.Subject}

	sub, err := i.nc.Subscribe(src.Subject, func(msg *nats.Msg) {
	...
	if err != nil {
		return &ErrSubscriptionFailed{Subject: src.Subject, Cause: err}
	}
