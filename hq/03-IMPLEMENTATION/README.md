...
| File | Role |
|---|---|
| [`roadmap.md`](roadmap.md) | The live plan: milestones, exit criteria, and the gate each milestone depends on. No dates — gates, not calendars. Shipped work lives in the ledger at the bottom, one line each, with its full story in the journey episode. |
...
- **Landing a feature means, in the same merge:** quality gate green
  (constitution), the roadmap item updated with the outcome, the journey
  episode written (`/journey-log`), and behavioral changes propagated into the
