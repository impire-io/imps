...
3. **Commit (never push).** Stage **only** `hq/01-RESEARCH/<slug>/` by explicit
   ...
   repository's standard co-author trailer. Pushing is the human's act; remind
   them research is always pushed (`git push`) so the trail exists even if the
   topic is later abandoned.
